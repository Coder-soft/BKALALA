(function($) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if ($.isEmptyObject(response.error)) {
                                $('.file-table' + id).remove(); // remove file
    ...
    $("body").on("click", "#saveInfoBtn", function(e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'storage': $('#storage').val(), // Home descritption
    ...
    // Update backblaze
    $("body").on("click", "#backblazeSaveBtn", function(e) {
        e.preventDefault();
        $(".backblazeBtn").prop("disabled", true); // disable button when click
        $.ajax({
            url: BASE_URL + '/admin/backblaze/update/store',
            type: "post",
            data: {
                'b2_bucket': $('#b2_bucket').val(), // b2_bucket
                'b2_account_id': $('#b2_account_id').val(), // b2_account_id
                'b2_application_key': $('#b2_application_key').val(), // b2_application_key
                'b2_url': $('#b2_url').val(), // b2_url
            },
            dataType: 'json',
            success: function(data) {
                $(".backblazeBtn").prop("disabled", false); // active button when on success
                if ($.isEmptyObject(data.error)) {
                    $(".print-error-msg").css('display', 'none'); // hide errors
                    swal("Success!", data.success, {
                        icon: "success",
    ...
    $("body").on("click", "#saveAdsBtn", function(e) {
        ...
        $.ajax({
            ...
            data: {
                ...
                'download_top_ads': $('#download_top_ads').val(), // download_top_ads
                'download_left_top_ads': $('#download_left_top_ads').val(), // download_left_top_ads
                'download_left_bottom_ads': $('#download_left_bottom_ads').val(), // download_left_bottom_ads
            },
            dataType: 'json',
            success: function(data) {
